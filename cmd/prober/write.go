...
func fulcioWriteEndpoint(ctx context.Context, priv *ecdsa.PrivateKey) (*x509.Certificate, error) {
	...
	var expectedCertCount int
	if local {
		expectedCertCount = 2
	} else {
		expectedCertCount = 3
	}
	if len(fulcioResp.CertificatesWithSct.CertificateChain.Certificates) != expectedCertCount {
		Logger.Errorf("unexpected number of certificates, got %d, expected %d",
			len(fulcioResp.CertificatesWithSct.CertificateChain.Certificates), expectedCertCount)
		return nil, err
	}
...
func rekorWriteEndpoint(ctx context.Context, cert *x509.Certificate, priv *ecdsa.PrivateKey) error {
	...
	for _, baseURL := range rekorV1URLs {
		...
		// TODO: Get Rekor public keys from trusted root, then verify the log entry using sigstore-go
