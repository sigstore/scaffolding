...
resource "google_monitoring_alert_policy" "k8s_pod_restart_failing_container" {
  # In the absence of data, incident will auto-close in 7 days
  ...
  display_name = "K8s Restart Failing Container Alert"
...
resource "google_monitoring_alert_policy" "k8s_pod_unschedulable" {
  # In the absence of data, incident will auto-close in 7 days
